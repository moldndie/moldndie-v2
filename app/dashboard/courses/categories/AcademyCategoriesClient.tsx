...
export default function AcademyCategoriesClient({ initialCategories }: Props) {
  ...
  function openEdit(cat: AcademyCategory) {
    setEditingId(cat.id)
    setForm({
      name:        cat.name,
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        if (editingId) {
          const updated = await updateAcademyCategory(editingId, {
            name:        form.name.trim(),
          ...
          const created = await createAcademyCategory({
            name:        form.name.trim(),
            description: form.description.trim() || null,
            sort_order:  Number(form.sort_order) || 0,
            is_active:   form.is_active,
  ...
  function handleDelete(id: string, name: string) {
    ...
    startTransition(async () => {
      try {
        await deleteAcademyCategory(id)
  ...
  function handleToggleActive(cat: AcademyCategory) {
    startTransition(async () => {
      try {
        await toggleAcademyCategoryActive(cat.id, !cat.is_active)
        setCategories((prev) =>
  ...
  function handleMove(id: string, direction: "up" | "down") {
    ...
    startTransition(async () => {
      try {
        await Promise.all([
          updateAcademyCategory(a.id, { name: a.name, description: a.description, sort_order: idx, is_active: a.is_active }),
          updateAcademyCategory(b.id, { name: b.name, description: b.description, sort_order: swap, is_active: b.is_active }),
        ])
        router.refresh()
      } catch {
        router.refresh()
      ...
      {showForm && (
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="acad_cat_active" className="text-sm text-zinc-600">Active</label>
          </div>
